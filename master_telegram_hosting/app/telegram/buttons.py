...
def main_menu_markup(is_admin: bool = False):
    rows = [
        [cb("📤 Upload Project", "menu:upload", "primary", "📤")],
        [cb("📁 My Projects", "menu:projects:0", "primary", "📁")],
    ]
    if is_admin:
        rows.append([cb("⚙️ Admin Panel", "admin:dashboard", "primary", "⚙️")])
    return markup(rows)
...
def projects_list_markup(project_ids: list[int], page: int, total_pages: int):
    ...
    for project_id in project_ids:
        rows.append([cb(f"Open Project #{project_id}", f"project:view:{project_id}", "primary", "📦")])
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Prev", f"menu:projects:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Next ➡️", f"menu:projects:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
    rows.append([cb("🏠 Main Menu", "menu:home", "primary", "🏠")])
    return markup(rows)
...
def project_actions_markup(project_id: int, status_value: str, is_admin: bool = False):
    ...
    if status_value in RUNNABLE_FROM_STOPPED:
        rows.append([
            cb("▶️ Start", f"project:start:{project_id}", "success", "▶️"),
            cb("🔄 Restart", f"project:restart:{project_id}", "primary", "🔄"),
        ])
    elif status_value in RUNNING_STATE:
        rows.append([
            cb("⏹ Stop", f"project:stop:{project_id}", "danger", "⏹"),
            cb("🔄 Restart", f"project:restart:{project_id}", "primary", "🔄"),
        ])

    if is_admin and status_value in PENDING_STATE:
        rows.append([
            cb("✅ Approve", f"approval:approve:{project_id}:0", "success", "✅"),
    ...
    rows.append([
        cb("📜 Logs", f"logs:view:{project_id}:stderr:all:0", "primary", "📜"),
    ])
    if is_admin:
        rows.append([cb("⚙️ Admin Panel", "admin:dashboard", "primary", "⚙️")])
    else:
        rows.append([cb("📁 Back to Projects", "menu:projects:0", "primary", "📁")])
    return markup(rows)
...
def approval_markup(project_id: int, queue_index: int, queue_total: int):
    ...
    nav = []
    if queue_index > 0:
        nav.append(cb("⬅️ Prev", f"admin:approvals:{queue_index - 1}", "primary", "⬅️"))
    if queue_index < queue_total - 1:
        nav.append(cb("Next ➡️", f"admin:approvals:{queue_index + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
    rows.append([cb("⚙️ Admin Panel", "admin:dashboard", "primary", "⚙️")])
    return markup(rows)
...
def admin_dashboard_markup():
    return markup(
        [
            [cb("📊 Dashboard", "admin:dashboard", "primary", "📊")],
            [cb("🕒 Approvals", "admin:approvals:0", "primary", "🕒")],
            [cb("👥 Users", "admin:users:0", "primary", "👥")],
            [cb("📦 Projects", "admin:projects_filter:all:0", "primary", "📦")],
            [cb("🧾 Activity", "admin:activity:0", "primary", "🧾")],
            [cb("🖥 System Stats", "admin:stats", "primary", "🖥")],
            [cb("🏠 Main Menu", "menu:home", "primary", "🏠")],
        ]
    )
...
def admin_users_markup(user_ids: list[int], page: int, total_pages: int):
    ...
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Prev", f"admin:users:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Next ➡️", f"admin:users:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
    rows.append([cb("⚙️ Admin Panel", "admin:dashboard", "primary", "⚙️")])
    return markup(rows)
...
def admin_projects_markup(project_ids: list[int], status_filter: str, page: int, total_pages: int):
    ...
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Prev", f"admin:projects_filter:{status_filter}:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Next ➡️", f"admin:projects_filter:{status_filter}:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
    rows.append([cb("⚙️ Admin Panel", "admin:dashboard", "primary", "⚙️")])
    return markup(rows)
...
def admin_user_detail_markup(telegram_user_id: int, is_banned: bool):
    action = cb("♻️ Unban", f"admin:unban_user:{telegram_user_id}", "success", "♻️") if is_banned else cb(
        "🚫 Ban", f"admin:block_user:{telegram_user_id}", "danger", "🚫"
    )
    return markup(
        [
            [action],
...
def user_projects_markup(project_ids: list[int], telegram_user_id: int, page: int, total_pages: int):
    ...
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Prev", f"admin:user_projects:{telegram_user_id}:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Next ➡️", f"admin:user_projects:{telegram_user_id}:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
...
def file_browser_markup(file_ids: list[int], project_id: int, page: int, total_pages: int):
    ...
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Prev", f"project:files:{project_id}:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Next ➡️", f"project:files:{project_id}:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
...
def file_view_markup(file_id: int, project_id: int, page: int, total_pages: int, editable: bool):
    ...
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Prev Page", f"file:view:{file_id}:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Next Page ➡️", f"file:view:{file_id}:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
    if editable:
        rows.append([
            cb("✏️ Replace", f"file:edit:{file_id}", "success", "✏️"),
...
def logs_view_markup(project_id: int, stream: str, filter_mode: str, page: int, total_pages: int):
    rows = [
        ...
        [
            ...
            cb("Errors/Warn", f"logs:view:{project_id}:{stream}:errors:0", "danger", "⚠️"),
        ],
    ]
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Newer", f"logs:view:{project_id}:{stream}:{filter_mode}:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Older ➡️", f"logs:view:{project_id}:{stream}:{filter_mode}:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
    rows.append([cb("📦 Back to Project", f"project:view:{project_id}", "primary", "📦")])
    return markup(rows)
...
def activity_markup(page: int, total_pages: int):
    ...
    nav = []
    if page > 0:
        nav.append(cb("⬅️ Prev", f"admin:activity:{page - 1}", "primary", "⬅️"))
    if page < total_pages - 1:
        nav.append(cb("Next ➡️", f"admin:activity:{page + 1}", "primary", "➡️"))
    if nav:
        rows.append(nav)
    rows.append([cb("⚙️ Admin Panel", "admin:dashboard", "primary", "⚙️")])
    return markup(rows)
